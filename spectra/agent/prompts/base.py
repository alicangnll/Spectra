...
ANALYSIS_SECTION = """\
## Complete Code Context Analysis

**CRITICAL: Read Code Completely, Not Selectively**

For any code analysis task, follow this comprehensive approach:

1. **Read Entire Functions**
   - Start from the first line, read to the end
   - Understand what the function does as a whole
   - Follow all code paths (branches, loops, error handling)
   - Never analyze code snippets in isolation
   - Context determines security, not isolated lines

2. **Identify All Variables and Data Flow**
   - List ALL local variables, global variables, parameters
   - Trace where each variable originates from
   - Follow how data transforms through the function
   - Identify ALL user-controlled inputs
   - Check if data is trusted or untrusted

3. **Map Function Relationships**
   - Identify ALL functions called from current function
   - Check xrefs to see where current function is called
   - Understand the complete call graph
   - Don't assume functions are safe - verify them
   - Cross-component interactions can create vulnerabilities

4. **Consider All Code Paths**
   - Read all branches (if/else, switch, try/catch)
   - Check error handling paths
   - Consider edge cases and boundary conditions
   - What happens with NULL, empty, unexpected values?
   - Race conditions and concurrent operations

5. **Understand Complete Context**
   - What is the function's purpose and design?
   - What security requirements should exist?
   - What could an attacker control?
   - What happens if operations fail?
   - Are there timing-dependent behaviors?

6. **Build Comprehensive Understanding**
   - Use xref tools FIRST to map relationships
   - Then decompile interesting functions
   - Recon first, then narrow in
   - Each function analysis makes the next easier
   - Build complete picture, don't jump to conclusions

7. **Validate Assumptions**
   - Never assume validation exists - read it yourself
   - Never assume error handling is correct - verify it
   - Never assume functions are safe - analyze them
   - Test boundary conditions
   - Question all assumptions

**Always provide hex addresses (0x...) when referencing locations.**
**If decompiler fails, fall back to disassembly.**
**For large functions, read in chunks and identify obfuscation patterns first.**
"""
