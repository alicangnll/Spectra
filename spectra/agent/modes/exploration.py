...
def _run_phase1_subagent(
    ...
    if kb.has_minimum_for_planning:
        ...
        if not _is_code_analysis:
            # This was not a code analysis task - silently skip exploration
    ...
    while state.phase == ExplorationPhase.EXPLORE:
        ...
        if state.explore_turns > state.max_explore_turns:
            if state.knowledge_base.has_minimum_for_planning and not explore_only:
                ...
                _is_code_analysis = any(
                    keyword in user_goal_lower
                    for keyword in ["analyze", "analiz", "bul", "find", "zafiyet", "vulnerability", "security"]
                )
...
def run_exploration_mode(
    ...
    # Early detection: Skip exploration mode for CLI directory mode without binary analysis context
    # CLI mode (dir_loc) doesn't have an IDB/BNDB path - exploration mode is for binary analysis only
    is_cli_mode = not loop.session.idb_path or loop.session.idb_path == ""
    ...
    if is_cli_mode:
        ...
        if not _is_explicit_analysis:
            # This is CLI mode but not explicitly asking for binary analysis
            # Skip exploration mode entirely for file creation, general questions, etc.
            log_info(f"CLI mode: Skipping exploration mode for non-binary-analysis task: {user_message[:60]!r}")
            return
    ...
    if not _is_exploration_task:
        log_info(f"Skipping exploration mode for non-code-analysis task: {user_message[:60]!r}")
        # Let the agent run normally without exploration mode
        return
