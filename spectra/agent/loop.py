...
class AgentLoop:
    ...
    def _is_dangerous_command(self, tool_name: str, args: dict[str, Any]) -> tuple[bool, str]:
        """Check if a tool call is dangerous and requires user approval.
        ...
        if tool_name == "shell_command":
            ...
            for sys_path in linux_macos_sensitive:
                if path.startswith(sys_path):
                    return True, f"File modification in system directory ({sys_path})"
    ...
    def _handle_spawn_subagent_tool(self, tc: ToolCall) -> Generator[TurnEvent, None, ToolResult]:
        ...
        if not task:
            ...
            try:
                # Notify UI that subagent is starting
                yield TurnEvent.subagent_spawned(
                    agent_id=agent_id,
                    name="Subagent",
                ...
                raw = yield from runner.run_task(task, max_turns=max_turns, silent=True)
                ...
                # Notify UI that subagent completed
                yield TurnEvent.subagent_completed(
                    agent_id=agent_id,
                    name="Subagent",
                ...
                yield TurnEvent.subagent_failed(
                    agent_id=agent_id,
                    name="Subagent",
                    error=str(e),
                )
