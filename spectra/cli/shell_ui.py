...
class ShellUI:
    ...
    def _handle_event(self, event: TurnEvent, pending_approval: str | None = None) -> Generator[None, None, None]:
        ...
        if event.type == TurnEventType.TEXT_DELTA:
            # Strip <think>...</think> reasoning first — it must never reach
            # the display buffer (or table re-render would leak it).
            text = self._filter_think(event.text)
            if not text:
                yield
                return
            ...
            # Close any still-open <think> span and flush held text first
            self._finish_think()
            ...
            self._in_bold = False
            self._in_code = False
            # Reset per-turn thinking state (keep _last_thinking)
            self._think_mode = False
            self._think_hold = ""
