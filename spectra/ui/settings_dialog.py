...
class SettingsDialog(QDialog):
    ...
    def _update_generation_defaults(self) -> None:
        ...
        for m in self._fetched_models:
            if m.id == model_id:
                ...
                # the user may have intentionally customized max_tokens /
                # context_window — don't overwrite them with the model's
                # defaults on every model-list refresh.
    ...
    def _build_update_tab(self) -> QWidget:
        ...
        self._update_progress_bar.setFormat("%p%")
        self._update_progress_bar.setStyleSheet(
            "QProgressBar { background-color: #222225; border: 1px solid #333338; "
            "border-radius: 4px; text-align: center; color: #d4d4d4; font-size: 11px; height: 16px; }"
            "QProgressBar::chunk { background-color: #3a9fd5; border-radius: 3px; }"
        )
    ...
    def _update_download_progress(self, downloaded: int, total: int) -> None:
        ...
        if total > 0:
            ...
            self._update_status_label.setText(f"Downloading update: {pct}% ({dl_mb:.1f} MB / {tot_mb:.1f} MB)")
        else:
            dl_mb = downloaded / (1024 * 1024)
    ...
    def customEvent(self, event) -> None:
        ...
        # Update checking uses QTimer-based polling, so no custom events are
        # dispatched currently. Kept as a hook for future use.
        pass
