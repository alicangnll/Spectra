...
from .styles import get_dark_theme, get_mode_bar_style, get_skill_chip_style
...
_SMALL_BTN_STYLE = (
    ...
    "QPushButton:hover { background: #323233; color: #ffffff; border-color: #4a4a4a; }"
    "QPushButton:pressed { background: #094771; }"
)
_CANCEL_BTN_STYLE = (
    "QPushButton { background: #3c1e1e; color: #f44747; border: 1px solid #5a2d2d; "
    "border-radius: 4px; padding: 4px 6px; font-size: 11px; font-weight: bold; min-height: 22px; }"
    "QPushButton:hover { background: #502525; color: #ffffff; }"
)
...
class SpectraPanelCore(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        # Top-level mode switcher: Chat | Tools | Functions.
        ...
        # --- Page 0: Chat ---
        chat_page = QWidget()
        chat_page.setStyleSheet("background-color: #181818;")
    ...
    def _build_input_section(self) -> QWidget:
        ...
        # Top Toolbar (Chips on left, Utility buttons on right)
        top_bar = QHBoxLayout()
        ...
        # Quick Skills Chips
        quick_skills = [
            ("vuln-audit", "/vuln-audit"),
            ("memory-corruption", "/memory-corruption"),
            ("plan", "/plan"),
            ("modify", "/modify"),
            ("explore", "/explore"),
        ]
        chip_style = get_skill_chip_style()
        for name, cmd in quick_skills:
            chip_btn = QPushButton(f"/{name}")
            chip_btn.setObjectName("skill_chip")
            chip_btn.setStyleSheet(chip_style)
            chip_btn.setToolTip(f"Insert {cmd} into input area")
            chip_btn.clicked.connect(lambda _, c=cmd: self._insert_skill_chip(c))
            top_bar.addWidget(chip_btn)

        top_bar.addStretch()

        # Utility Buttons (New, Export, Settings, Tools, Agent+)
        ...
        self._mutations_btn.setVisible(False)
        top_bar.addWidget(self._mutations_btn)
        ...
        top_bar.addWidget(self._tools_btn)
        ...
        top_bar.addWidget(self._create_agent_btn)

        v_layout.addLayout(top_bar)

        # Main Input Row (Input text area + Send/Stop buttons)
        ...
        # Primary Action Stack (Send / Stop)
        send_stack = QVBoxLayout()
        send_stack.setSpacing(4)
        send_stack.setContentsMargins(0, 0, 0, 0)
        ...
        send_stack.addWidget(self._send_btn)
        ...
        send_stack.addWidget(self._cancel_btn)

        input_layout.addLayout(send_stack)
        v_layout.addLayout(input_layout)

        return self._input_container

    def _insert_skill_chip(self, cmd: str) -> None:
        current = self._input_area.toPlainText()
        if not current.strip():
            self._input_area.setPlainText(f"{cmd} ")
        else:
            self._input_area.setPlainText(f"{current.rstrip()} {cmd} ")
        ...
        try:
            ...
            if mutation_panel is not None:
                ...
                if hasattr(mutation_panel, "shutdown"):
                    try:
                        ...
                        log_debug(f"Mutation panel shutdown failed: {e}")
            ...
            if hasattr(self, "_functions_next_btn"):
                try:
                    ...
                    log_debug(f"Functions next_btn disconnect failed: {e}")
