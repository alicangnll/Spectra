...
if !ARM64! equ 1 (
    echo [*] Windows ARM64 detected: Using pre-built binary packages where available...
)
...
if defined BN_PYTHON (
    ...
    call :install_with_python "%BN_PYTHON%" "%REQ%"
    if !errorlevel! equ 0 (
...
if defined BN_FOUND_PYTHON (
    ...
    call :install_with_python "!BN_FOUND_PYTHON!" "%REQ%"
    if !errorlevel! equ 0 (
        set "PYTHONHOME="
...
if !errorlevel! equ 0 (
    ...
    call :install_with_python "python3" "%REQ%"
...
if !errorlevel! equ 0 (
    ...
    call :install_with_python "python" "%REQ%"
    if !errorlevel! equ 0 exit /b 0
)
...
:: ── Subroutine: install_with_python ─────────────────────────────
:install_with_python
set "PY_CMD=%~1"
set "REQ_FILE=%~2"
...
if !ARM64! equ 1 (
    ...
    %PY_CMD% -m pip install cryptography --only-binary=:all: >nul 2>&1
    if !errorlevel! neq 0 (
        %PY_CMD% -m pip install "cryptography>=41.0.0" >nul 2>&1
    )
)
