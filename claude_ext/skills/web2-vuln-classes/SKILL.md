...
## ⚠️ NOVELTY FILTER — Before Using This Reference

**This skill documents KNOWN vuln classes.** Use it as a reference, NOT a checklist.

**Before testing any class below, run the 0day-first filter:**

```
┌─────────────────────────────────────────────────────────────┐
│           "AM I FINDING A NOVEL BUG OR A KNOWN ONE?"       │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  Step 1: Search CVE/Exploit-DB for this pattern             │
│  └─ Found? → LOW bounty potential. (Documented)            │
│  └─ Not found? → HIGH potential. (Novel)                   │
│                                                               │
│  Step 2: Would scanners/tools find this?                    │
│  └─ Yes (nuclei, sqlmap, XSStrike)? → Saturated.            │
│  └─ No (requires manual logic)? → Unique.                   │
│                                                               │
│  Step 3: Does this require feature combination?              │
│  └─ Yes (A + B → bug)? → CRITICAL. (Feature interaction)   │
│  └─ No (single endpoint)? → Check saturation.              │
│                                                               │
│  Step 4: Is the target code recent (<6 months)?              │
│  └─ Yes → UNAUDITED. High 0day potential.                  │
│  └─ No → Check for prior patches/disclosures.               │
│                                                               │
└─────────────────────────────────────────────────────────────┘
```

**Use this skill for:**
- ✅ Understanding root causes (to find similar NEW patterns)
- ✅ Bypass techniques (to overcome WAFs/validators)
- ✅ Chain opportunities (escalate low → critical)
- ❌ NOT as a testing checklist (that = N-Day hunting)
...
| Frida | Runtime hooking of unserialize() |
