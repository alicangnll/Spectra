...
## CRITICAL RULES (ZERO-DAY PRIORITY UPDATED)
...
### NEW: ZERO-DAY FIRST HUNTING (2025 UPDATE)

**Before ANY checklist testing, run novelty filter:**

```
┌─────────────────────────────────────────────────────────────┐
│              NOVELTY FILTER DECISION TREE                    │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  1. Does this pattern exist in CVE/Exploit-DB?               │
│     └─ YES → Documented. Low bounty potential.              │
│     └─ NO  → Novel. HIGH priority.                           │
│                                                               │
│  2. Would automated scanners find this?                     │
│     └─ YES → Saturated. Deprioritize.                        │
│     └─ NO  → Unique. HIGH priority.                          │
│                                                               │
│  3. Does this require combining 2+ features?                 │
│     └─ YES → Complex. HIGH priority (feature interaction).   │
│     └─ NO  → Simple. Check saturation.                       │
│                                                               │
│  4. Is this in recent code (< 6 months)?                     │
│     └─ YES → Unauiudited. CRITICAL priority.                 │
│     └─ NO  → Old code. Check for prior patches.             │
│                                                               │
└─────────────────────────────────────────────────────────────┘
```

**Vuln Class Priority (0day-first ordering):**

| Priority | Vuln Class | Why Novel? |
|----------|-----------|------------|
| 🔴 CRITICAL | Feature interaction bugs | Unique per app, scanners can't find |
| 🔴 CRITICAL | Business logic bypass | Context-specific, requires deep understanding |
| 🔴 CRITICAL | Race conditions (financial ops) | Requires timing, hard to automate |
| 🟠 HIGH | Cache poisoning protocols | Complex, requires cache key analysis |
| 🟠 HIGH | Protocol state machine abuse | Requires understanding expected flow |
| 🟡 MEDIUM | GraphQL/OAuth/OIDC chains | Can be novel if unique combo |
| 🟡 MEDIUM | CI/CD pipeline bugs | Requires workflow analysis |
| 🟢 LOW | IDOR | Well-documented, high competition |
| 🟢 LOW | XSS | Saturated, automated tools exist |
| 🟢 LOW | SSRF (basic) | Known IP bypass techniques documented |
| ⚪ SKIP | SQLi (classic) | Too documented, unless novel blind technique |
