...
verify_symlinks() {
    ...
    # Check if symlinks exist and point to valid locations
    if [[ ! -L "$PLUGINS_DIR/spectra" ]]; then
        err "Symlink not created: $PLUGINS_DIR/spectra"
        ((errors++))
    elif [[ ! -d "$PLUGINS_DIR/spectra" ]]; then
        err "Symlink points to non-existent directory: $PLUGINS_DIR/spectra"
        ((errors++))
    fi

    if [[ ! -L "$PLUGINS_DIR/spectra_plugin.py" ]]; then
        err "Symlink not created: $PLUGINS_DIR/spectra_plugin.py"
        ((errors++))
    elif [[ ! -f "$PLUGINS_DIR/spectra_plugin.py" ]]; then
        err "Symlink points to non-existent file: $PLUGINS_DIR/spectra_plugin.py"
        ((errors++))
    fi
...
verify_builtin_skills() {
    local builtin_dir="$SCRIPT_DIR/spectra/skills/builtins"

    if [[ ! -d "$builtin_dir" ]]; then
        warn "Built-in skills directory not found: $builtin_dir"
        return 0
    fi

    local skill_count=$(find "$builtin_dir" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')
    info "Built-in skills available: $skill_count"
    ...
    if [[ $skill_count -gt 0 ]]; then
        ...
        for skill_dir in "$builtin_dir"/*/; do
            if [[ -d "$skill_dir" && -f "$skill_dir/SKILL.md" ]]; then
                ...
                local desc=$(head -3 "$skill_dir/SKILL.md" | grep "description:" | sed 's/.*description: //' | cut -c1-60)
                echo "  • $slug: ${desc}..."
                ((count++))
                if [[ $count -ge 5 ]]; then
    ...
    local critical_skills=("0day-find" "android-exploit" "ssl-pinning-bypass" "app-shielding-bypass")
    local missing=0
    for skill in "${critical_skills[@]}"; do
        if [[ ! -d "$builtin_dir/$skill" ]]; then
            warn "Missing critical skill: $skill"
            ((missing++))
        fi
    done

    if [[ $missing -gt 0 ]]; then
        err "Missing $missing critical skill(s)"
        return 1
    fi
...
if ! verify_builtin_skills; then
    warn "Some built-in skills are missing, but installation will continue"
fi
...
test_python_import() {
    ...
    # Find Python to use for testing
    local test_python=""
    if [[ -n "${IDA_PYTHON:-}" ]]; then
        test_python="$IDA_PYTHON"
    elif command -v python3 >/dev/null 2>&1; then
        test_python="$(command -v python3)"
    else
        warn "Could not find Python for import test"
        return 0
    fi

    # Test import skills_tab
    local test_output
    if test_output=$("$test_python" -c "
import sys
sys.path.insert(0, '$SCRIPT_DIR')
try:
    from spectra.ui.tabs.skills_tab import SkillsTab
    print('✓ skills_tab.py import OK')
except Exception as e:
    print(f'✗ skills_tab.py import failed: {e}')
    sys.exit(1)
" 2>&1); then
        ...
        warn "skills_tab.py import test failed"
        echo "$test_output"
    fi
...
try:
    ...
    print(f'✓ Skill discovery OK: {count} skills')
except Exception as e:
...
" 2>&1); then
        ...
        warn "Skill discovery test failed"
        echo "$test_output"
    fi
}
...
echo ""
info "Built-in skills are loaded from: $SCRIPT_DIR/spectra/skills/builtins/"
info "User skills directory: $SKILLS_DIR/"
echo ""
...
echo ""
warn "If skills don't appear in Settings, restart IDA Pro to reload the plugin."
